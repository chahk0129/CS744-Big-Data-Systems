...
if [  "$1" = "clear" ]; then
	...
	hdfs dfs -rm -r ${namenode_dir}/user/hcha/assignment1/web-BerkStan.partition
	hdfs dfs -rm -r ${namenode_dir}/user/hcha/assignment1/enwiki-pages-articles.partition
...
if [ "$1" = "web-BerkStan" ]; then
	...
	echo "Running pagerank_partition with web-BerkStan.txt"
	echo "num_partition(${num_partition}), executor_mem(${executor_memory})" >> logs/web-BerkStan.log
	spark-submit --master spark://$ip:7077 --class "pagerank_partition" --driver-memory ${driver_memory} --executor-memory ${executor_memory} --num-executors 2 --executor-cores 5 pagerank_partition.py ${assignment1_dir}/web-BerkStan.txt ${assignment1_dir}/web-BerkStan.partition ${num_partition} >> logs/web-BerkStan.log
else
	...
	echo "Running pagerank_partition enwiki-pages-articles"
	echo "num_partition(${num_partition}), executor_mem(${executor_memory})" >> logs/enwiki-pages-articles.log
	spark-submit --master spark://$ip:7077 --class "pagerank_partition" --driver-memory ${driver_memory} --executor-memory ${executor_memory} --num-executors 2 --executor-cores 5 pagerank_partition.py ${assignment1_dir}/enwiki-pages-articles ${assignment1_dir}/enwiki-pages-articles.partition ${num_partition} >> logs/enwiki-pages-articles.log
fi
...
## Read output file
if [ "$1" = "web-BerkStan" ]; then
	hdfs dfs -cat ${assignment1_dir}/web-BerkStan.partition/* 2>/dev/null | head
else
	hdfs dfs -cat ${assignment1_dir}/enwiki-pages-articles.partition/* 2>/dev/null | head
fi
